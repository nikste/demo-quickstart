...
object JobMiniClusterTest {
  ...
  def main(args: Array[String]) {
    ...
    val miniClusterStream = new LocalFlinkMiniCluster(config, false, StreamingMode.STREAMING)
    val miniClusterBatch = new LocalFlinkMiniCluster(config, false, StreamingMode.BATCH_ONLY)

    miniClusterStream.start()
    miniClusterBatch.start()





    miniClusterBatch.stop()
    miniClusterStream.stop()
